...
Purpose: using API 1, Find imdb ID for the Netflix titles missing the same
"""
...
missing_imdb = find_missing_imdbid('api1')
...
# Search by title and go through the 1st page only of the matches returned
# Each page contains 10 matches
url = "https://movie-database-imdb-alternative.p.rapidapi.com/"
querystring = {"page":"1","r":"json","s":"title name"}
headers = {
    ...
    if response.status_code == 200:
        ...
        if js['Response'] == 'True':
            ...
            for item in range(0,total_items):
                ...
                if js['Search'][item]['Title'].lower().replace(" ", "") != title.lower().replace(" ", ""): continue
                tupl = (flix_id, js['Search'][item]['imdbID'], 'api1-found')
                found_by_api1 = found_by_api1 + 1
                missing_imdb_result.append(tupl)
                break
        if len(tupl) < 1:
            tupl = (flix_id, '', 'api1-not found')
            missing_imdb_result.append(tupl)

# Update the netflix tables with the imdbID found by the API
if len(missing_imdb_result) > 0: execute_sql2(missing_imdb_result)
if search_limit > 0:
    print('IMDB IDs searched by API 1:', len(missing_imdb_result))
    print('IMDB IDs found by API 1:', found_by_api1)
